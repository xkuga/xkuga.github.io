...
当你需要操作一个 key 的时候，他会通过哈希去计算这个 key 落到了环状的哪一个结点上，
然后会重定向到那个结点进行操作。
...
运行的时候我们需要给容器分配 IP，而分配置 IP 需要先创建一个网络，参数要根据你的情况修改。
...
命令是意思是使用 onepiece 这个网络，并给容器分配 10.10.10.100 这个 IP。
...
--replicas 2 表示集群 1 个主结点使用 2 个从结点，
...
这里有主从关系，但是这个不直观，所以我写了一个脚本处理了一下，代码如下。
...
关于这个问题，GitHub 上有相关的 [issue](https://github.com/antirez/redis/issues/2527)，
...
重复的不说了，这个就是一个 ugly 的方法哈哈。

## 未解决的问题
-------------

虽然容器内的网络是通的，但如果要把集群提供给外部使用，一旦发生重定向，就会失败，
原因是外部和容器的网络不通。留个问题，后面解决，这也有相关的 [issue](https://github.com/antirez/redis/issues/2527)。
今天先写到这，以后再填，好累 XD。
